...
window.addEventListener("DOMContentLoaded", function () {
	...
	if (map !== null) {
		window.addEventListener("scroll", function (e) {
			...
			if (
				scrollFromTop >= scrollMap &&
				map.getAttribute("data-js-map-loaded") === "false"
			) {
	...
	function initMap() {
		...
		let marker, i, s; // add all locations icon in map
		...
		let asideMarker;
		locationButtons.forEach(function (el, indx) {
			// show pharmacy
			...
			if (copyBtn.length > 0) {
				copyBtn[indx].addEventListener("click", function (elm) {
					getMapStaticImg(elm);
				});
			}
		});
		...
		function showMarkerOnMap(data, dataEl) {
			...
			const parent = document.querySelector(
				'[data-js-map-item-parent="'.concat(id, '"]')
			);
			const parents = document.querySelectorAll("[data-js-map-item-parent]"); // create new marker
			...
			asideMarkers.push(asideMarker); // change marker icon

			asideMarkers.forEach(function (markers) {
			...
			asideMarker.setIcon(activeIcon); // remove active class

			parents.forEach(function (e) {
				return e.classList.remove(
					"map-iframe-aside__pharmacy-list-item--active"
				);
			}); // add active class
		...
		function getMapStaticImg(data) {
			...
			const addNewMapInfoHead = document.querySelector(
				"[data-js-add-pharmarcy-btn-head]"
			);
			const addNewMapInfoBottom = document.querySelector(
				"[data-js-add-pharmarcy-btn-bottom]"
			);
			const tel = [];
			const email = [];
			...
			for (let key = 0; key < pharmacyTel.length; key++) {
				...
				tel.push(
					'<a class="basket-cloned-pharmacy-info__link" href="tel:'
						.concat(telEdited, '">')
						.concat(pharmacyTel[key], "</a>")
				);
			}
			...
			for (let key = 0; key < pharmacyEmail.length; key++) {
				...
				email.push(
					'<a class="basket-cloned-pharmacy-info__link" href="tel:'
						.concat(emailEdited, '">')
						.concat(pharmacyEmail[key], "</a>")
				);
			}

			addNewMapInfoHead.classList.add(
				"basket-checkout-box__add-map-info-btn--hidden"
			);
			addNewMapInfoBottom.classList.add(
				"basket-cloned-pharmacy-info__btn--visible"
			);
			input.value = pharmacyID;
			wrapper.innerHTML = "";
			wrapper.insertAdjacentHTML(
				"afterbegin",
				'\n\t\t\t\t\t\t<div class="basket-cloned-pharmacy-info__col">\n\t\t\t\t\t\t\t<p class="basket-cloned-pharmacy-info__address-title">\xDCnvan</p>\n\t\t\t\t\t\t\t<h5 class="basket-cloned-pharmacy-info__pharmacy-title">'
					.concat(
						pharmacyName,
	...
	function initBasketMap() {
		// start latLng
		let myLatlng = new google.maps.LatLng(40.410519, 49.867317); // styles for map
		...
		let map = new google.maps.Map(
			document.getElementById("map-basket"),
			mapOptions
		); // add marker
		...
		google.maps.event.addListener(map, "click", function (event) {
			...
			let clickedLocation = event.latLng; //If the marker hasn't been added
		...
		function markerLocation() {
			...
			let input = document.querySelector(
				"[data-js-basket-map-info-for-courier]"
			); // add latlng to input

			input.value = ""
				.concat(currentLocation.lat(), ",")
				.concat(currentLocation.lng());
		}
	}
});
